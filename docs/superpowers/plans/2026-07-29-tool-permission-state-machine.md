...
2026-07-29 已在解锁的 macOS 桌面完成最终手动验收。测试使用只更新临时后端内存计数的 `computer.example` 假工具，不包含真实电脑操作：

- `system.current_time` 返回 `succeeded`，Electron 未显示确认卡片。
- 高风险卡片正确显示工具名称、影响、过期时间，并将敏感参数显示为 `[REDACTED]`。
- 点击“拒绝”后请求状态为 `rejected`，处理函数执行计数保持为 0。
- 点击“允许一次”后请求状态为 `succeeded`，重复提交同一确认后执行计数仍为 1。
- 保留未过期确认并重启 Electron 后，确认卡片通过待确认列表恢复。

验收结束后已清理临时确认请求和测试进程，并恢复正式本机后端与 QQ 连接。
